<?php
  session_start();
    ...
    {
        $stmt = mysqli_prepare($con, "DELETE FROM `user_queries` WHERE `sr_no`=?");
    ...
    {
        $stmt = mysqli_prepare($con, "DELETE FROM `user_queries`");
        if(mysqli_stmt_execute($stmt)) {
            $res = mysqli_stmt_affected_rows($stmt);
            mysqli_stmt_close($stmt);
            return $res;
           ...
           <table class="table table-hover border">
                ...
                <tbody>
                    ...
                    while($row = mysqli_fetch_assoc($data))
                    {
                        ...
                        $seen.="<a href='?del=$row[sr_no]' class='btn btn-sm rounded-pill btn-danger mt-2' data-id='$row[sr_no]'>Delete</a>";
                        echo<<<query
                        <tr>
                            <td>$i</td>
                            <td>$row[name]</td>
