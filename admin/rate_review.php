<?php
  session_start();
    ...
    {
        $stmt = mysqli_prepare($con, "DELETE FROM `rating_review` WHERE `sr_no`=?");
    ...
    {
        $stmt = mysqli_prepare($con, "DELETE FROM `rating_review`");
        if(mysqli_stmt_execute($stmt)) {
            $res = mysqli_stmt_affected_rows($stmt);
            mysqli_stmt_close($stmt);
            return $res;
...
<head>
    ...
    <title>Admin Pannel - Ratings & Reviews</title>
      ...
      <div class="card border-0 shadow mb-4" >
        ...
        <div class="text-end mb-4">
           ...
           <table class="table table-hover border">
                ...
                <tbody>
                    <?php
                    $q = "SELECT rr.*,uc.name AS uname, r.name AS
